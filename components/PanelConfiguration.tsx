  ...
  return (
    <div className="max-w-6xl mx-auto bg-slate-800 rounded-xl p-8 border border-slate-700">
      <div className="text-center mb-8">
        ...
        <p className="text-slate-400">Gemini has generated these agents based on your target role. Customize them to fit your specific needs.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        {panelists.map((p, index) => (
          <div key={p.id} className={`bg-slate-900 rounded-lg border-t-4 border-${p.avatarColor}-500 flex flex-col shadow-lg`}>
            <div className="p-4 border-b border-slate-800 bg-slate-900/50">
                <div className="flex items-center gap-3 mb-2">
                    <div className={`w-10 h-10 rounded-full bg-${p.avatarColor}-600 flex items-center justify-center font-bold text-white`}>
                        {p.name[0]}
                    </div>
                    <div>
                        <input 
            ...
            <div className="p-4 space-y-4 flex-grow">
                <div>
                    <label className="text-xs text-slate-500 uppercase font-bold block mb-1">Primary Focus</label>
                    <input 
                        value={p.focus}
                ...
                <div>
                    <label className="text-xs text-slate-500 uppercase font-bold block mb-1">Behavior & Style</label>
                    <textarea 
                        value={p.description}
      ...
      <div className="bg-blue-900/20 border border-blue-800 p-4 rounded-lg mb-8 text-sm text-blue-200 flex items-center gap-2">
          ...
          <span><strong>Tip:</strong> You can edit the "Behavior & Style" to change how aggressive, technical, or casual the agent acts during the live interview.</span>
      </div>

      <div className="flex justify-center">
        <button 
            onClick={onConfirm}
            className="px-8 py-3 bg-green-600 hover:bg-green-700 text-white font-bold rounded-lg transition-transform hover:scale-105 shadow-[0_0_15px_rgba(22,163,74,0.4)]"
        >
            Confirm Panel & Join Room
        </button>
      </div>
    </div>
  );
